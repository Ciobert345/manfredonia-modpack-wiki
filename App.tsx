...
const App: React.FC = () => {
  ...
  const [selectedPlatform, setSelectedPlatform] = useState<'all' | 'modrinth' | 'curseforge'>('all');
  ...
  const filteredMods = useMemo(() => {
    return ALL_MODS.filter(mod => {
      ...
      const matchesPlatform = selectedPlatform === 'all' ||
        (selectedPlatform === 'modrinth' && isMN) ||
        (selectedPlatform === 'curseforge' && isCF);

      return matchesSearch && matchesCategory && matchesLibraryFilter && matchesPlatform;
    });
  }, [searchTerm, selectedCategory, showLibraries, selectedPlatform]);
    ...
    <div className="min-h-screen flex flex-col md:flex-row text-white font-['Inter'] bg-transparent relative selection:bg-white selection:text-black">
      ...
      <aside className="hidden md:flex flex-col w-[310px] shrink-0 h-screen sticky top-0 bg-black/40 backdrop-blur-3xl border-r border-white/10 z-[110] shadow-[inset_-20px_0_40px_rgba(0,0,0,0.4)]">
            ...
            {CATEGORIES.map((cat, idx) => {
              ...
              return (
                <button
                  ...
                  className={`w-full relative flex items-center gap-4 px-5 py-4 text-[11px] rounded-2xl transition-all duration-500 group overflow-hidden ${isActive
                    ? 'bg-white text-black font-black shadow-[0_10px_25px_rgba(255,255,255,0.1)]'
                    : 'text-white/40 hover:text-white hover:bg-white/[0.04]'
                    }`}
                >
                  {/* Status Indicator */}
                  <div className={`absolute left-0 top-1/2 -translate-y-1/2 w-1 h-1/2 rounded-full transition-all duration-500 ${isActive ? 'bg-black' : 'bg-transparent group-hover:bg-white/20'
                    }`} />
        ...
        <div className="p-8 border-t border-white/10 bg-black/20">
          <button
            ...
            className={`w-full flex items-center justify-between px-5 py-4 rounded-2xl border transition-all duration-500 ${showLibraries
          ...
          >
            ...
            <div className={`w-2 h-2 rounded-full transition-all duration-500 ${showLibraries
      ...
      <main className="flex-grow flex flex-col relative min-w-0">
          ...
          <div className="px-6 md:px-12 flex items-center gap-5">
            ...
            <div className="relative flex-grow">
              <div className={`flex items-center backdrop-blur-md border rounded-2xl transition-all duration-500 ${searchTerm ? 'bg-black/40 border-white/40 shadow-[0_0_30px_rgba(255,255,255,0.05)]' : 'bg-black/20 border-white/10 hover:bg-black/40'}`}>
                ...
                {/* INTEGRATED PLATFORM FILTERS */}
                <div className="hidden lg:flex items-center gap-1 pr-2 ml-2 py-2">
                  <div className="w-px h-6 bg-white/10 mr-2" />
                  {[
                    { id: 'all', icon: LayoutGrid, tooltip: 'ALL' },
                    { id: 'modrinth', icon: Hexagon, tooltip: 'MN' },
                    { id: 'curseforge', icon: Zap, tooltip: 'CF' }
                  ].map(p => (
                    <button
                      key={p.id}
                      onClick={() => setSelectedPlatform(p.id as any)}
                      className={`w-10 h-10 flex items-center justify-center rounded-xl transition-all duration-300 group relative ${selectedPlatform === p.id
        ...
        <div className={`md:hidden fixed inset-0 z-[999] bg-black/60 backdrop-blur-3xl transition-all duration-500 flex flex-col pt-28 px-4 pb-6 overflow-y-auto ${isMenuOpen ? 'opacity-100 translate-y-0 pointer-events-auto' : 'opacity-0 -translate-y-4 pointer-events-none'}`}>
          ...
          {/* PLATFORM SELECTOR IN MENU */}
          <div className="mb-6 grid grid-cols-3 gap-2">
            {[
              { id: 'all', label: 'ALL', icon: LayoutGrid },
              { id: 'modrinth', label: 'MN', icon: Hexagon },
              { id: 'curseforge', label: 'CF', icon: Zap }
            ].map(p => (
              <button
                key={p.id}
                onClick={() => { setSelectedPlatform(p.id as any); setIsMenuOpen(false); }}
                className={`flex flex-col items-center justify-center gap-2 py-3 rounded-xl border transition-all duration-300 ${selectedPlatform === p.id
            ...
            {CATEGORIES.map(cat => {
              ...
              return (
                <button
                  ...
                  className={`relative w-full flex items-center gap-3 px-3 py-3 rounded-xl border transition-all duration-300 group overflow-hidden ${isActive
                    ? 'bg-white text-black border-white shadow-[0_4px_20px_rgba(255,255,255,0.2)]'
          ...
          <div className="mt-auto pt-6 border-t border-white/10 space-y-3">
            <button
              ...
              className={`w-full flex items-center justify-between px-4 py-3 rounded-xl border transition-all duration-300 ${showLibraries
